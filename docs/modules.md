...
- **文件结构**：`index.ts` / `routes.ts` / `service.ts` / `validator.ts` / `types.ts`
- **Service**：`UserConfigService` —— `get(userId, prefetchedRow?)`、`getRawRow(userId)`、`set(userId, panelJson, searchEngineJson)`、`updatePanel(userId, panelJson)`、`updateSearchEngine(userId, searchEngineJson)`。
...
- **API**：
  ...
  - 分组：`saveGroup` → `/panel/itemIconGroup/edit`、`deleteGroups` → `/panel/itemIconGroup/deletes`、`saveGroupSort` → `/panel/itemIconGroup/saveSort`
...
- **职责**：随机一言 API（前端直连上游，不走后端代理）。
- **文件**：`api.ts` / `types.ts`
- **API**：
  - `getHitokoto(params?)` → 直连 `params.apiUrl`（默认 `https://v1.hitokoto.cn/`），GET + 3s 超时，返回裸 `HitokotoData`（`{ id, hitokoto, from, from_who, uuid, type }`）
- **类型**：`HitokotoParams`、`HitokotoData`

#### music 模块

- **职责**：音乐解析 API（前端直连上游 Meting，不走后端代理）。
- **文件**：`api.ts` / `types.ts`
- **API**：
  - `parseMusic(params)` → 直连 `params.apiUrl`（Meting API），列表抓取 + 分批 10 首并发抓 url/pic，3s 超时，最多 50 首，返回裸 `MusicTrack[]`（每项含 `{ id, name, artist, album, url, pic, source }`）
- **类型**：`MusicServer`、`MusicType`、`MusicParseParams`、`MusicTrack`

#### search 模块

- **职责**：搜索引擎配置 API。封装对后端 `user-config` 模块的写入，方便页面/store 复用，本身不暴露独立后端路由。
- **文件**：`api.ts` / `types.ts`
- **API**：
  - `saveSearchEngineConfig(config)` → `POST /panel/userConfig/set`，请求体 `{ searchEngine: config }`（实际落盘由后端 `user-config` 模块处理，访客模式只读返回 403）
- **类型**：`SearchEngine`、`SearchEngineConfig`（与 `@/modules/panel/types` 复用同一形状，便于在首页与设置弹窗间共享）
