...
- **模块名 / 挂载路径**：`hitokoto` / `/panel/hitokoto`（路由内部定义 `/get`，完整路径 `POST /panel/hitokoto/get`）
- **职责**：随机一言代理 + 缓存，支持用户自定义上游 API 地址（默认 `https://v1.hitokoto.cn/`）。前端"一言"功能通过此接口获取句子。
- **模块级中间件**：`router.use('*', publicModeMiddleware)` —— 访客可读。
- **路由**：
  - `POST /panel/hitokoto/get` —— 入参 `apiUrl?` / `categories?` / `minLength?` / `maxLength?`（均可选）；路由层 `Cache-Control: private, max-age=60`。
- **文件结构**：`index.ts` / `routes.ts` / `service.ts` / `validator.ts` / `types.ts`
- **Service**：`getHitokoto(params)` —— `isValidUrl(apiUrl)` SSRF 校验 → isolate 级 `Map` 内存缓存（TTL 5 分钟，key 含 `apiUrl`）→ 上游 GET 请求（3s 超时，响应 ≤ 4KB）→ 标准化为 `{ id, hitokoto, from, from_who, uuid, type }`。
- **依赖 shared**：`middleware/auth`（`publicModeMiddleware`）、`validate`、`response`、`errors`（`AppError`）、`favicon`（`isValidUrl`）。
- **注册**：`registry.register(hitokotoModule)`

#### music 模块

- **模块名 / 挂载路径**：`music` / `/panel/music`（路由内部定义 `/parse`，完整路径 `POST /panel/music/parse`）
- **职责**：Meting API 代理 + 缓存，解析用户配置的歌单/单曲为可播放的曲目列表。前端"音乐播放器"功能通过此接口拿曲目元信息（含真实 url/pic）。
- **模块级中间件**：`router.use('*', publicModeMiddleware)` —— 访客可读。
- **路由**：
  - `POST /panel/music/parse` —— 入参 `server` / `type` / `id` / `apiUrl`（必填）；路由层 `Cache-Control: private, max-age=60`。
- **文件结构**：`index.ts` / `routes.ts` / `service.ts` / `validator.ts` / `types.ts`
- **Service**：`parseMusic(params)` —— `isValidUrl(apiUrl)` SSRF 校验 → isolate 级 `Map` 内存缓存（TTL 30 分钟，key = `apiUrl|server|type|id`）→ 列表抓取（3s 超时，响应 ≤ 512KB，取前 50 项）→ 每首 `url`/`pic` 用 `Promise.allSettled` 并发抓取（分批 10 首，单首同样 3s 超时 + 仅 GET）→ 标准化为 `MusicTrack[]`（`{ id, name, artist, album, url, pic, source }`）。
- **依赖 shared**：`middleware/auth`（`publicModeMiddleware`）、`validate`、`response`、`errors`（`AppError`）、`favicon`（`isValidUrl`）。
- **注册**：`registry.register(musicModule)`
...
- **文件**：`api.ts` / `types.ts`
- **API**：
  - `getHitokoto(params?)` → `POST /panel/hitokoto/get`，参数 `apiUrl?` / `categories?` / `minLength?` / `maxLength?`，返回 `HitokotoData`（`{ id, hitokoto, from, from_who, uuid, type }`）
- **类型**：`HitokotoParams`、`HitokotoData`

#### music 模块

- **职责**：音乐解析 API（基于 Meting）。
- **文件**：`api.ts` / `types.ts`
- **API**：
  - `parseMusic(params)` → `POST /panel/music/parse`，参数 `server` / `type` / `id` / `apiUrl`，返回 `MusicTrack[]`（每项含 `{ id, name, artist, album, url, pic, source }`）
- **类型**：`MusicServer`、`MusicType`、`MusicParseParams`、`MusicTrack`
